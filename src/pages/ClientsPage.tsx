...
import { Building2, Plus, ChevronDown, ChevronRight, CheckCircle2, Clock, X, ListTodo } from 'lucide-react';
...
export default function ClientsPage() {
  ...
  const createTaskMutation = useMutation({
    mutationFn: async ({ clientName, task }: { clientName: string; task: typeof newTask }) => {
    ...
    onSuccess: () => {
      ...
      setShowNewTask(null);
      toast.success('Tarefa criada para o cliente!');
    },
        ...
        <div className="rounded-2xl border border-brand-orange/20 bg-white/[0.03] p-5">
          <div className="flex items-center gap-3">
            <input
              ...
              className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-brand-orange/40"
                ...
                {isExpanded && (
                  <div className="border-t border-white/5 p-5 space-y-3">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button
                        onClick={() => setShowNewTask(showNewTask === client.id ? null : client.id)}
                    ...
                    {showNewTask === client.id && (
                      <div className="rounded-xl border border-brand-orange/20 bg-white/[0.02] p-4 space-y-3">
                        <input
                          value={newTask.title}
                          onChange={(e) => setNewTask({ ...newTask, title: e.target.value })}
                          placeholder="Título da tarefa..."
                          className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-brand-orange/40"
                        />
                        <textarea
                          ...
                          rows={2}
                          className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-brand-orange/40 resize-none"
                        />
                        <div className="flex gap-3">
                          <select
                            value={newTask.priority}
                            onChange={(e) => setNewTask({ ...newTask, priority: e.target.value })}
                            className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-brand-orange/40"
                          >
                          ...
                          />
                          <button
                            onClick={() => newTask.title.trim() && createTaskMutation.mutate({ clientName: client.name, task: newTask })}
                            className="ml-auto px-4 py-2 rounded-lg bg-brand-orange text-white text-sm font-medium hover:bg-brand-orange/90 transition-colors"
                          >
                            Criar
                          </button>
                        </div>
